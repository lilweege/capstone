...
def normalize_token(tok_type, tok_string):
...
def hash_token(token: Token) -> int:
...
def tokenize_file(file_path: str) -> tuple[list[Token], set[str]]:
    """
        ...
        with open(file_path, 'rb') as f:
            ...
            for tok in token_generator:
                ...
                if tok.type in (tokenize.COMMENT, tokenize.NL, tokenize.NEWLINE, tokenize.ENCODING):
                    continue
                normalized = normalize_token(tok.type, tok.string)
...
def compute_rolling_hashes(tokens: list[Token], k: int) -> list[Fingerprint]:
    """
    ...
    h = 0
    for i in range(k):
        token_val = hash_token(tokens[i])
        h = (h * BASE + token_val) % MOD
    hashes.append(Fingerprint(hash_val=h, position=0))
    
    for i in range(1, n - k + 1):
        left_token_val = hash_token(tokens[i - 1])
        h = (h - left_token_val * high_order) % MOD
        h = (h * BASE + hash_token(tokens[i + k - 1])) % MOD
        hashes.append(Fingerprint(hash_val=h, position=i))

    return hashes


def winnowing(tokens: list[Token], k: int, w: int) -> dict[int, int]:
    """
    ...
    """
    kgram_hashes = compute_rolling_hashes(tokens, k)
    n = len(kgram_hashes)
...
def index_files(file_paths: list[str], k: int, w: int) -> tuple[dict[str, set[int]], dict[str, set[str]]]:
    """
    ...
    for file_path in file_paths:
        tokens, comments = tokenize_file(file_path)
        fingerprints = winnowing(tokens, k, w)
...
def report_similarity(file_fingerprints: dict[str, set[int]], file_comments: dict[str, set[int]], min_common_percent: float) -> list[tuple[str, str, float]]:
    """
...
def main():
    ...
    file_fingerprints, file_comments = index_files(args.files, k=args.k, w=args.w)
    
    similarities = report_similarity(file_fingerprints, file_comments, min_common_percent=args.m)
