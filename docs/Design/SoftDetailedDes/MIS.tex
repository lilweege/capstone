...
\begin{itemize}
    \item \texttt{Module}: Export of Model Object.
\end{itemize}
...
\begin{itemize}
    \item The input source code is in one programming language.
\end{itemize}
\subsubsection{Access Routine Semantics}
...
\textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
\hline
\texttt{tokenize} & string & list[Token] & TokenizeError \\
\hline
\end{tabular}
\end{center}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item None
    \end{itemize}
    \item \textbf{output:} Returns a single token read the given index in the source string, or None if invalid.
    % \item \textbf
\end{itemize}
...
\section{MIS of Similarity Scoring Module} \label{SimScoreModule}
